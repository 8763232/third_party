...
std::string ASCIIToUTF8(const StringPiece& ascii) {
	DCHECK(IsStringASCII(ascii)) << ascii;
	return WideToUTF8(string16(ascii.begin(), ascii.end()));
}
...
std::string WideToASCII(const string16& utf16)
{
	return SysWideToNativeMB(utf16);
}

std::wstring ASCIIToWide(const StringPiece& ascii)
{
	return SysNativeMBToWide(ascii);
}
