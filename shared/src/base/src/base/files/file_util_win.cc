...
bool DeleteFileTo(const FilePath& path, bool recursive) {
  ThreadRestrictions::AssertIOAllowed();
...
#pragma warning(suppress:4996)  // don't complain about wcscpy deprecation
  ...
  int err = SHFileOperationW(&file_operation);
...
bool CopyDirectory(const FilePath& from_path, const FilePath& to_path,
  ...
  while (success && !current.empty()) {
    ...
    } else if (!CopyFileTo(current, target_path)) {
...
bool CopyFileTo(const FilePath& from_path, const FilePath& to_path) {
  ThreadRestrictions::AssertIOAllowed();
...
bool CopyAndDeleteDirectory(const FilePath& from_path,
  ...
  if (CopyDirectory(from_path, to_path, true)) {
    if (DeleteFileTo(from_path, true))
      return true;
